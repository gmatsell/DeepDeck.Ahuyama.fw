...
static void wifi_initialize_receive(void){
...
static void espnow_recv_cb(const esp_now_recv_info_t *recv_info, const uint8_t *data, int data_len){
	...
	if(data_len == MATRIX_ROWS*MATRIX_COLS){
		...
		xQueueSend(espnow_receive_q,(void*)&CURRENT_MATRIX, (TickType_t) 0);
	}
...
static void espnow_initialize_receive(void){
...
void espnow_receive(void){
	...
	wifi_initialize_receive();
	espnow_initialize_receive();
}
